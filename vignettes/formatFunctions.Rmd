...
First, we load the package and generate a mock summarised result.
...
This functions helps to manipulate estimate_name and estimate_value columns. For instance, if we want that all the variables for which we have counts and percentage to be display in a single row showing both as "N (%)" we can do it with this function. The `estimateNameFormat` is where all combinations or renaming of estimates can be specified. Values from estime_name's column should be specified between <...>. The new estimate_name values to use will be the names of the vector, or the value itself when it is not named.
