...
Tables displayed in `visOmopResults()` follow a default style, but customisation is possible through the `.options` argument. This argument allows users to modify various formatting aspects using options from the `format` functions (see the *`format` Functions* section to learn more).
...
In the examples above, we used the default style defined in the *visOmopResults* package (use `gtStyle()` and `flextableStyle()` to see these styles). However, it's possible to customise the appearance of different parts of the table to better suit your needs.
