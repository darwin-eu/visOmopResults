...
Similar to `splitStrata`, the functions `splitGroup` will split *group_name* and *group_level* columns, while `splitAdditional` will split the additional name-level pair. Finally, the function `splitAll` will split group, strata, and additonal at once.

A part from the summarised_result object, another argument to these functions is `overall`. This argument is to specify whether to keep the *overall* column when splitting. It is set to FALSE by default. Now, we repeat the previous example setting "overall" to TRUE. Notice how the output has one more column which refers to the overall strata.
...
#### !! Keyword: and
Looking at the results below, observe how the splitting was not only done by values in the "name" column, but also within values containing the key word "**and**". That is, "sex and age_group" was splitted into *sex* and *age_group* columns, instead of generating a column called "*sex and age_group*".
...
A part from the columns to unite argument (`cols`), there is the argument `removeNA`, defaulted to TRUE. This means that, if a row of the targeted column contain only "NA" values, it will be united as "overall", while if `removeNA` is set to FALSE, it will join them as if NA was a value. In the next example we use `removeNA` set to FALSE:
...
Now, if we set `removeNA` to TRUE, we obtain an output where only names and levels of non-NA values are returned, and fro those rows where all values are NA it uses "overall".
```{r}
to_unite_strata |>
  uniteStrata(cols = c("age", "sex", "region"))
```
