...
Similar to `splitStrata`, the functions `splitGroup` will split *group_name* and *group_level* columns, while `splitAdditional` will split the additional name-level pair. Finally, the function `splitAll` will split group, strata, and additonal at once. Note that after using `splitStrata` on our summarised_result object, we do no longer have a *strata_name-strata_level* pair, instead we have two new columns corresponding to the stratifications, *age_group* and *sex*.
