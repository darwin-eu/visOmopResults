...
Wheras, by default (`ignore = c(NA, "overall")`) we obtain an output where only names and levels of non-NA values are returned, and from those rows where all values are NA it uses "overall".
```{r}
to_unite_strata |>
  uniteStrata(cols = c("age", "sex", "region"))
```
