...
To work with summarised_result objects, we have the `uniteGroup`, `uniteStrata`, and `uniteAdditional` functions which will generate the group, strata, and additional name-level columns respectivelly from a given set of columns. For instance, in the following example we want to create the *group_name* and *group_level* columns:
...
Lastly, the function `uniteNameLevel`, idem to `splitNameLevel`, provides more flexibility on the name-level column naming, in additon of the `keep` argument (FALSE by default) to choose whether to keep the targeted columns. For instance, if we repeat the previous example with `keep` set to TRUE we would obtain the following output:
