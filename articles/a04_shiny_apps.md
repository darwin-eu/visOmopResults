...
by sex strata and provide panel-specific visualisation options:
...
[here](https://github.com/darwin-eu/visOmopResults/blob/main/inst/shinyApp/app.R).
...
When a `_brand.yml` file is available, it can be used to style both the
Shiny interface and the tables and plots produced with
**visOmopResults** and packages that build on it.

For example, the brand can be applied to the Shiny interface using
**bslib**:
...
Similarly, **visOmopResults** can use the styling defined in
`_brand.yml` when generating tables and plots. This helps maintain a
consistent appearance between the Shiny interface and the results
displayed within it.
...
The examples above demonstrate how **visOmopResults** can be used to
build a custom Shiny application. When the aim is instead to quickly
create an application for exploring OMOP results, we can use the
[OmopViewer](https://CRAN.R-project.org/package=OmopViewer) R package.

**OmopViewer** supports results from a range of OMOP-related R packages,
as well as other results stored in the `summarised_result` format.

For example, a static application can be generated from existing results
using:
...
When creating a static application, **OmopViewer** generates an R
project containing the Shiny application and its source code. This
provides a useful starting point that can then be modified to meet the
specific needs of a study, for example by changing the layout, adding
filters, or adapting how results are displayed.

For further customisation of tables and plots in the generated
application, **visOmopResults** can be particularly useful. Its table
and plotting functions can be used to modify existing visualisations or
create new ones while maintaining a consistent approach to displaying
OMOP results.

In this way, **OmopViewer** can be used to quickly generate a Shiny
application, while **visOmopResults** provides additional flexibility
when more tailored visualisations or application components are
required.
