...
test_that("formatSummarisedResult, spanHeader", {
  result <- mockSummarisedResult()
...
test_that("formatSummarisedResult, gtTable", {
  ...
  # no style
  # spanner styles
  expect_null(gtResult$`_styles`$styles[gtResult$`_styles`$grpname %in% gtResult$`_spanners`$spanner_id[gtResult$`_spanners`$spanner_level %in% c(1,3)]] |>
                 unlist() |> unique())
  expect_null(gtResult$`_styles`$styles[gtResult$`_styles`$grpname %in% gtResult$`_spanners`$spanner_id[gtResult$`_spanners`$spanner_level == 2]] |>
                        unlist() |> unique())
  # title and subtitle
