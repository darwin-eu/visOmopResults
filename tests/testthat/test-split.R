test_that("uniteGroups", {
  ...
  expect_no_error(res <- tib |> uniteNameLevel("z", keep = TRUE))
  expect_true("group_name" %in% colnames(res))
  expect_true("group_level" %in% colnames(res))
  expect_true("z" %in% colnames(res))
  expect_no_error(res <- tib |> uniteNameLevel("z", name = "new_col", level ="new_col2"))
  expect_true("new_col" %in% colnames(res))
  expect_true("new_col2" %in% colnames(res))
  expect_no_error(res <- tib |> uniteNameLevel(c("z", "a")))
  expect_true("group_name" %in% colnames(res))
  expect_true("group_level" %in% colnames(res))
  ...
  expect_identical(
    ...
    tib |> uniteNameLevel(c("z", "a")) |> splitGroup()
  )
  expect_error(tib |> uniteNameLevel(c("x")))
  expect_error(tib |> dplyr::rename("x and x" = "x") |> uniteNameLevel(c("x and x")))
  expect_warning(res <- tib |> uniteNameLevel("z", name = "x"))
})
